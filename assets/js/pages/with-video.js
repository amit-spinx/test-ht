document.addEventListener('DOMContentLoaded', () => {
  ...
  const onDragStart = (event) => {
    if (!dragEnabled) return;  // Prevent drag if disabled
    isDragging = true;
  ...
  const animateFrames = (direction) => {
    isAnimating = true;
    dragEnabled = false; // Disable drag while animating
    let framesToAnimate = maxFramesPerDrag;
    const interval = setInterval(() => {
      if (framesToAnimate <= 0) {
        clearInterval(interval);
        isAnimating = false;
        dragEnabled = true; // Re-enable drag after animation
        return;
      }
  ...
  const onDragMove = (event) => {
    if (!isDragging || isAnimating || !dragEnabled) return;  // Prevent move if disabled
  ...
  document.querySelectorAll('.open-trtment-modal').forEach(function(btn) {
    btn.addEventListener('click', function(e) {
      ...
      const rowProcedure = document.getElementById('viewProcedure');
      if (modal) {
        isModalOpen = true;
        ...
        rowProcedure.classList.add('modal-parent-active');
        ...
        dragEnabled = false; // Disable drag when modal is open
        // Get video URL from data-video
        var videoUrl = btn.getAttribute('data-video');
        var videoElement = document.getElementById('activePartsVideo');
  ...
  document.querySelectorAll('.treatment-modal .close-modal').forEach(function(btn) {
    btn.addEventListener('click', function() {
      ...
      setTimeout(() => {
        ...
        rowProcedure.classList.remove('modal-parent-active');
        // Remove active class from all triggers with same data-target
        document.querySelectorAll('.open-trtment-modal').forEach(function(trigger) {
          trigger.classList.remove('modal-active');
        });
        isModalOpen = false;
        dragEnabled = true; // Re-enable drag after modal closes
        var videoElement = document.getElementById('activePartsVideo');
        if (videoElement) {
          videoElement.pause();
          videoElement.removeAttribute('src'); // Reset for next time
          videoElement.load();
        }
      }, 400);
    });
